...
const ShoppingCartController = {
  addToCart: async (req, res, next) => {
    ...
    try {
      ...
      if (
        !items.find(
          (value) => value.toString() === itemId && item.id === itemId
        )
      )
        throw new ErrorHandle(404, 'Not found item in product')

      const subtotal = subTotal(price, quantity)
      await ShoppingCart.create({
        products: [{ productId, itemId, quantity, subtotal }]
      })
        .then((cart) => {
  ...
  pushOnCart: async (req, res, next) => {
    ...
    try {
      ...
      const { products } = cart
      const subcart = products
        .map((element) => element)
        .find(
          (value) =>
            value.productId._id.toString() === productId &&
            value.itemId._id.toString() === itemId
        )
      if (subcart) {
        const { _id } = subcart
        ...
        await ShoppingCart.findOneAndUpdate(
          { 'products._id': _id },
          { 'products.$.quantity': quantity, 'products.$.subtotal': subtotal },
        ...
        if (
          ...
          throw new ErrorHandle(400, 'Not found item on product')
        const subtotal = subTotal(price, quantity)
        await ShoppingCart.findByIdAndUpdate(
          cartId,
          { $push: { products: { productId, itemId, quantity, subtotal } } },
          { new: true, runValidators: true }
        )
          .then((cart) => {
