...
describe('ProductController', () => {
  ...
  beforeEach(async () => {
    const resSeller = await request(server).post('/auth/login').send({
      email: 'mariarodriguez@gmail.com',
      password: 'password'
    })
  ...
  describe('FIND', () => {
    ...
    it('Debe de devolver un producto', async () => {
      const res = await request(server).get(
        '/products/product/5f67940fc155976374b99876'
      )
      expect(res.statusCode).toBe(200)
    ...
    it('Debe de devolver un articulo', async () => {
      const res = await request(server).get(
        '/products/items/5f67940fc155976374b99877'
      )
      expect(res.statusCode).toBe(200)
    ...
    it('Debe de devolver un error al devolver un producto', async () => {
      const res = await request(server).get(
        '/products/product/5f67940fc155976374b99879'
      )
      expect(res.statusCode).toBe(404)
      expect(res.body.message).toBe('Not found product')
    })
    ...
    it('Debe de devolver el producto por parent_sku', async () => {
      const res = await request(server).get('/products/search?parentsku=CENE00')
      expect(res.statusCode).toBe(200)
      expect(res.body.products).toHaveLength(1)
    })
  })
